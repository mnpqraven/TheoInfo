...
$= (a)^{-1} (((0 + 0) + (0+ a))((abb)? (ab)^* ba)^*)$ CHECK NGOAC DONG NAY
...
Beispiel: $G \Rightarrow a\mathcal{U}$
...
$L(P_3) = \{ a^m b^n c^p d^q$ $|$ $m,n,p,q \in \mathbb{N}_0, m \geq q, n \leq p, m+ n = p + q\}$

Beispiel: Ableitung von $a^5 b^2 c^4 d^3 6 L(P_3)$
...
- Wir wiederholen das Gleicher mit $e := b$
...
- Die nicht markierten Position in der oberen tabelle zeigen, welche Zustände äquivalent sind. Hier bestehen die Äquivalenzklassen von $\sim$ aus $\{q_0,q_4,q_8\},\{q_1, q_5\},\{q_3,q_7\},\{q_2\}$. Das Automat A ist schon minimal.
...
Es gilt: für jedes Zeichen $a \in \Sigma_A$ ist $\{a\}$ eine reguläre Sprache.

(Fall 1 in der Definition von regulären Sprachen). Daraus folgt: Für jedes Wort $w \in \Sigma^*$ ist $\{w\}$ auch eine reguläre Sprache, denn für $w = q_1 q_2 ... q_n$ gilt $\{w\} = \{a_1\} \circ \{a_2\} \circ ... \circ \{a_n\}$

(nach Fall 2 in der Definition von regulären Sprachen). Sei $L = \{w_1, w_2, ..., w_m\} = \{w_1\} \cup \{w_2\} \cup ... \cup \{w_m\}$ und der oben gezeigten Eigenschaft gilt: L ist eine reguläre Sprache
