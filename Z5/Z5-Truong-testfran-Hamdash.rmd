...
-  $n$ ist aber auch die Anzahl der Äquivalenzklassen($|R_L|$ - der Index der Sprache L) von L, denn diese $n$ Worte sind paarweise L-trennbar.
...
Also $\forall w \in \Sigma^*: \delta_{A\times B}^*((p,q),w) = (\delta_A^*(p,w),\delta_B^*(q,w))$
